...
function curlToPHP(curl) {
	var err = 'if (status!= CURLcode.CURLE_OK) throw new Exception(curl.StrError(status));\n';
	var endCurl = 'curl.Cleanup();\n';
	var promo = "// Generated by curl-to-csharp: https://github.com/ixfo/curl-to-sharp/";
	var start = "Curl.GlobalInit((int)CURLinitFlag.CURL_GLOBAL_ALL);\nstring res=null;\nEasy.WriteFunction wf = new Easy.WriteFunction((Byte[] buf, Int32 size, Int32 nmemb, Object extraData) =>\n{ res+= System.Text.Encoding.UTF8.GetString(buf);return size * nmemb;});\nvar curl = new Easy();\n";
	var result = 'var status=curl.Perform();';
	...
	var code = promo+"\n"+start;
    code += 'curl.SetOpt(CURLoption.CURLOPT_URL, '+phpExpandEnv(req.url)+');\ncurl.SetOpt(CURLoption.CURLOPT_WRITEFUNCTION, wf);\ncurl.SetOpt(CURLoption.CURLOPT_SSL_VERIFYHOST, 1);\ncurl.SetOpt(CURLoption.CURLOPT_SSL_VERIFYPEER, 0);\n';
	...
	function renderSimple(method) {
		var php = "";
		if (method == "POST")
			php = 'curl.SetOpt(CURLoption.CURLOPT_POST, 1);\n';
		else if (method == "HEAD")
			php = 'curl.SetOpt(CURLoption.CURLOPT_CUSTOMREQUEST, \'HEAD\');\ncurl.SetOpt(CURLoption.CURLOPT_NOBODY, true);\n';
		else if (method != "GET")
			php = 'curl.SetOpt(CURLoption.CURLOPT_CUSTOMREQUEST, '+phpExpandEnv(method)+');\n';
	...
	function renderComplex(req) {
		...
		if (req.data.ascii || req.data.files) {
			...
			if (ioReaders.length == 1 && typeof varName == 'undefined') {
				php += 'curl.SetOpt(CURLoption.CURLOPT_POSTFIELDS, '+ioReaders[0]+');\n';
				...
				php += ioReaders.join(",\n    ");
				php += '\n);\n';
				php += 'curl.SetOpt(CURLoption.CURLOPT_POSTFIELDS, $post);\n';
			}
		}
		if (req.method == "POST")
			php += 'curl.SetOpt(CURLoption.CURLOPT_POST, 1);\n';
			...
			php += 'curl.SetOpt(CURLoption.CURLOPT_CUSTOMREQUEST, '+phpExpandEnv(req.method)+');\n\n';

		// set basic auth
		if (req.basicauth) {
			php += 'curl.SetOpt(CURLoption.CURLOPT_USERPWD, '+phpExpandEnv(req.basicauth.user)+' + ":" + '+phpExpandEnv(req.basicauth.pass)+');\n';
		}

		// set compressed
		if (req.compressed) {
			php += 'curl.SetOpt(CURLoption.CURLOPT_ENCODING, "gzip, deflate");\n';
		}
		...
		for (var name in headers) {
			...
			phpHeader += 'headers.Append('+phpExpandEnv(name).slice(0,-1)+': '+phpExpandEnv(headers[name]).slice(1)+');\n';
		}
		if (typeof phpHeader != 'undefined') {
			php += phpHeader + 'curl.SetOpt(CURLoption.CURLOPT_HTTPHEADER, headers);\n';
		}
		delete phpHeader;
